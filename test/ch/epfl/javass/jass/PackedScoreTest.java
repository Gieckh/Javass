...
class PackedScoreTest {
    ...
    @Test //Runs in around 42s.
    void isValidReturnsTrueWithAllValidTricks() { //Symmetric
        for (int i1 = 0 ; i1 <= MAX_TRICKS_PER_TURN ; ++i1) {
            for (int j1 = 0 ; j1 <= MAX_POINTS_PER_TURN ; j1+=17) {
                for(int k1 = 0 ; k1 <= MAX_POINTS_PER_GAME ; k1+=68) {
                    for (int i2 = 0 ; i2 <= MAX_TRICKS_PER_TURN; ++i2) {
                        for(int j2 = 0 ; j2 <= MAX_POINTS_PER_TURN ; j2 += 13) {
                            for(int k2 = 0; k2 <= MAX_POINTS_PER_GAME ; k2 += 77) {
